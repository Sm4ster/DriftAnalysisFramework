...
export default {
  ...
  mounted() {
    ...
    this.transformContextMatrix(this.ctx, this.viewbox, window.devicePixelRatio, 'fill')
  },
  ...
  watch: {
    ...
    viewbox(viewbox) {
      this.transformContextMatrix(this.ctx, viewbox, window.devicePixelRatio, 'fill')
      this.ctx.clearRect(-this.canvas.width, -this.canvas.height, 2 * this.canvas.width, 2 * this.canvas.height);
      this.draw_data();
    },
