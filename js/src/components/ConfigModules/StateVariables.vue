<template>
  <div class="space-y-5">
    <div class="space-y-6">
      ...
      <SingleStateVariable v-for="state_variable in definition" :ref="state_variable.code"
                            :init="defaults(state_variable)"
                            :validation="validation(state_variable)"
      >
        <template v-slot:title>
          <div :class="[state_variable.bottom_bias ? 'mt-1.5': '']">${{ state_variable.symbol }}$</div>
        </template>
...
export default {
  ...
  methods: {
    ...
    defaults(variable) {
      ...
      return {...defaults, ...variable.defaults,  "variation_enabled": variable.variation_enabled};
    },
    validation(variable) {
      let validation = {};
      Object.entries(variable.fields).map(e => {
